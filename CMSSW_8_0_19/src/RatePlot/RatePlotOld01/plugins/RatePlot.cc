...
//   	
	...
	for (unsigned int k = 0; k < digis->size(); k++) {
		...
		if (kDigi.code() < 7 && kDigi.code() > 3) {
			if(kDigi.stNum()!=1) continue;
...
//			std::cout << "iPhi: " << iphi << " pT:" << getPt(kDigi) << std::endl;
			...
			for (edm::SortedCollection<HOTriggerPrimitiveDigi,
						...
						hoPrimitivesHandle->begin(); it != hoPrimitivesHandle->end();
						++it) {
					const HOTriggerPrimitiveDigi rh = *it;
					if(abs(rh.iphi() - iphi) > 1) continue;
					if(!isInWheel(rh.ieta(), kDigi.whNum())) continue;
					...
					for (l1t::RegionalMuonCandBxCollection::const_iterator it =
							...
							if((abs(rh.iphi() - iphiTrack) < 2) && ((rh.ieta() - ietaTrack) < 2)) isPresent = true;
					//		std::cout << "BMTF: " << mc.hwPt() * 0.5 << " GeV; "
					//				<<  << " rad; "
					//				<< mc.hwEta() * 0.010875 << std::endl;
					}
					
					if(!isPresent) addTracks->Fill(getPt(kDigi));
...
void RatePlot::beginJob() {
	...
	addTracks = fs->make < TH1D
				> ("AddTracks", "AddTracks", 40, 0, 40);
	
	bmtfTracks = fs->make < TH1D
					> ("BmtfTracks", "BmtfTracks", 40, 0, 40);
...
bool RatePlot::isInWheel(int ieta, int wheel){
	if(ieta < -10 && wheel == -2) return true;
	if((ieta < -4 && ieta > -11) && wheel == -1) return true;
	if((ieta < 5 && ieta > -5) && wheel == 0) return true;
	if((ieta > 4 && ieta < 11) && wheel == 1) return true;
	if(ieta > 10 && wheel == 2) return true;
	return false;
}
